...
import {
  ADD_RELATIONS_ACTION,
  DELETE_TILE_ACTION,
  LOAD_TILE_MAP_ACTION,
  RETRIEVE_TILE_MAP_ACTION,
  _TileMapMapActionTypes,
  actionFactory,
} from './tilemap-actions';
...
export class TileMapActionListener extends ActionListener<
  TileMapState,
  ActionEvent<_TileMapMapActionTypes, ActionPayload>
> {
  constructor() {
    super(
      (state: TileMapState, actionEvent: ActionEvent<_TileMapMapActionTypes, ActionPayload>) => {
...
if (state) {
  tileMapActionListener.subscribeToState(state).subscribe(async (value) => {
    await appStore.tileMapStore.setProp('TILES', 'tiles', value.tiles);
  });
}
