...
export class IsometricMapView {
  tileDefWidth: number = 100;
  tileDefHeight: number = 58;
  tileDefHeightOffesetY: number = 4;

  innerJoin(arrays: (number[] | undefined)[]): number[] {
  ...
  //TODO: by weight
  getRandomElement(arr: number[]): number | undefined {
    if (arr.length === 0) {
  ...
  debugGenerateMap(cols: number, rows: number, tileMapId: TileMapId) {
    const map = tileMapId
  ...
  async generateMap(cols: number, rows: number, tiles: Tile[]): Promise<TileMapId> {
    ...
    this.debugGenerateMap(cols, rows, tileMapId);
  ...
  drawTiles(
    ...
    const _dX = -3 * this.tileDefWidth + offsetX;
    const _dY = 0 + offsetY;
    ...
    for (let j = 0; j < rows; j++) {
      for (let i = 0; i < cols; i++) {
        ...
        y = y + i * (this.tileDefHeight / 2) * Math.cos(3.14 / 2);
        x = x + j * (this.tileDefWidth / 2) * Math.sin(3.14 / 2);
        ...
        this.drawPoint(ctx, _x + _dX, _y + _dY, 'blue');
...
const isometricMapView = new IsometricMapView();

export async function main() {
  tileMapActionListener.dispatch(
    actionFactory('load_tile_map_from_file', { jsonPath: 'TODO:PATH' }),
  );

  appStore.tileMapStore.observeProp('TILES', 'tiles').subscribe(async (tiles) => {
    setTimeout(async () => {
      console.log(tiles);
      await appStore.tileMapStore.setProp('TILES', 'tileMapIds', [[3]]);
    }, 1000);
  });
  ...
  appStore.tileMapStore.observeProp('TILES', 'tiles').subscribe(async (tiles) => {
    ...
    if (ctx && tiles) {
      isometricMapView.clearCanvas(ctx);

      const cols = 20;
      const rows = 50;
      const tileMapIds = await isometricMapView.generateMap(cols, rows, tiles);

      //appStore.tileMapStore.set('TILES', t);
      await appStore.tileMapStore.setProp('TILES', 'tileMapIds', tileMapIds);
      //await appStore.tileMapStore.setProp('TILES', 'tiles', tiles.tiles);

      //const cols = 34;
      //const rows = 220;
      isometricMapView.drawTiles(cols, rows, tiles, tileMapIds, ctx, 0, 0);
    }
  });
  ...
  appStore.inputDevicesStore.getObservable('DEVICE').subscribe(async (state: InputDevicesState) => {
    ...
    if (ctx && _tiles) {
      ...
      isometricMapView.drawTiles(cols, rows, _tiles.tiles, _tiles.tileMapIds, ctx, ++offX, ++offY);
